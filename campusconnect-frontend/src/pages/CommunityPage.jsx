    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 via-white to-blue-50 px-4 py-12">
      ...
      <div className="w-full max-w-4xl space-y-6 animate-fadeIn">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-blue-700 mb-2">CampusConnect Community</h1>
          <p className="text-gray-500">Connect, share, and engage with your campus community</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xl">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <textarea
                placeholder={showPollCreator ? "What's your poll question?" : "What's on your mind?"}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="poll-mode" className="flex items-center gap-2 text-sm text-gray-700">
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">{post.createdBy?.name || 'Anonymous'}</p>
                ...
                {/* Post Content */}
                <div className="mb-4">
                  <p className="text-gray-800 leading-relaxed">{post.content || post.text || ''}</p>
                  {post.image && (
                ...
                {/* Poll */}
                {post.type === 'poll' && post.poll && Array.isArray(post.poll.options) && (
                  <div className="mb-4 p-4 bg-blue-50 rounded-lg">
                    <h4 className="font-medium text-gray-900 mb-3">{post.poll.question || post.text}</h4>
                    <div className="space-y-2">
                          ...
                          <div key={i} className="relative">
                            <button
                              className={`w-full flex justify-between text-left p-3 rounded-lg border ${
                                hasVoted ? 'bg-blue-100 border-blue-300' : 'border-gray-300'
                              } hover:bg-blue-50 transition`}
                              onClick={() => handleVote(post._id || post.id, opt.option)}
                    ...
                    <p className="text-sm text-gray-600 mt-2">
                      Total votes: {post.poll.options.reduce((sum, o) => sum + (o.votes?.length || 0), 0)}
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {reactions.map((reaction) => {
                      const reactionCount = post.reactions?.[reaction.name]?.length || 0;
                      const hasReacted = post.reactions?.[reaction.name]?.includes(currentUser?._id);

                      return (
                        <button
                          key={reaction.name}
                          className={`flex items-center gap-1 px-2 py-1 rounded-full text-sm ${
                            hasReacted ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'
                          } transition`}
                          onClick={() => handleReaction(post._id || post.id, reaction)}
                        >
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <div className="flex-1 flex gap-2">
                    <input
                      ...
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
                          handleComment(post._id || post.id, comments[post._id || post.id]);
                        }
                      }}
                      className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                    />
                    <button
                      onClick={() => handleComment(post._id || post.id, comments[post._id || post.id])}
                      disabled={!currentUser || !comments[post._id || post.id]?.trim()}
